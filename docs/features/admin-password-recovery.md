...
docker exec mium-master /app/bin/mium-cli.sh iam:reset-password
```
...
```bash
NEW_PW=$(docker exec mium-master /app/bin/mium-cli.sh iam:reset-password)
```
...
mium.admin.socket.enabled = false
